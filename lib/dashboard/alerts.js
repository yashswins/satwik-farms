  ...
  for (const issue of catalogSnapshot?.payload?.issues || []) {
    if (issue.kind === 'item_disabled_in_erp' || issue.kind === 'item_missing_in_erp') {
      ...
      alerts.push({
        ...
        detail: `Sheet ${issue.sheet_price?.toLocaleString()} per ${issue.unit} (${issue.expected_rate?.toLocaleString()} per stock unit) vs ${issue.invoiced_rate?.toLocaleString()} invoiced, ${issue.drift_pct > 0 ? '+' : ''}${issue.drift_pct}% over ${issue.lines} lines in the last 14 days.`,
        threshold: 'median invoiced rate more than 5% and TSH 100 from the Sheet, at least 3 lines',
