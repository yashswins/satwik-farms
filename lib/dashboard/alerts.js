  ...
  for (const issue of catalogSnapshot?.payload?.issues || []) {
    if (issue.kind === 'item_disabled_in_erp' || issue.kind === 'item_missing_in_erp') {
      ...
      alerts.push({
        ...
        detail: `Sheet ${issue.sheet_price?.toLocaleString()} per ${issue.unit} (${issue.expected_rate?.toLocaleString()} per stock unit) vs ${issue.invoiced_rate?.toLocaleString()} on the latest ${issue.lines} app lines${issue.latest_line ? `, newest ${issue.latest_line}` : ''} (${issue.drift_pct > 0 ? '+' : ''}${issue.drift_pct}%).`,
