...
/** 'Thu 4 Sep' for a single day, 'Mon 1 Sep – Thu 4 Sep' for a range. */
...
/**
 * Dar calendar date (YYYY-MM-DD) of a timestamp that may arrive as a Date
 * object (the database driver's timestamps) or as a string. Naive strings
 * are UTC, as everything the backend writes is.
 */
export function dateOnly(value) {
  if (!value) return '';
  const d = value instanceof Date
    ? value
    : new Date(/[zZ]|[+-]\d\d:?\d\d$/.test(String(value)) ? value : `${String(value).replace(' ', 'T')}Z`);
...
// Three channels. Orders from before 2026-09-03, when the backend started
// recording app vs web, are attributed to the app (not one web order has been
// seen since recording began), so there is no fourth "unsplit" bucket.
...
export const CHANNEL_COLORS = {
  app: '#53B175',
  web: '#2F6FB5',
  online_unsplit: '#53B175',
  offline: '#FF8A65',
};

export const CHANNELS = ['app', 'web', 'offline'];
