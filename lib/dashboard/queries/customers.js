...
const DISPLAY = 'COALESCE(o.customer_name, c.contact_name, c.customer_name, x.customer)';
const APP_NAME_LATERAL = `LEFT JOIN LATERAL (
  ...
  return query(
    `WITH x AS (
       SELECT customer,
              ...
              COUNT(*) FILTER (WHERE channel IN ('app', 'web', 'online_unsplit'))::int AS online_invoices
...
export async function migration(start, end) {
  return one(
    `WITH per AS (
       SELECT customer,
              BOOL_OR(channel IN ('app', 'web', 'online_unsplit')) AS any_online,
              BOOL_OR(COALESCE(channel, 'offline') = 'offline') AS any_offline,
              MIN(posting_date) FILTER (WHERE channel IN ('app', 'web', 'online_unsplit')) AS first_online,
...
export async function search(q, limit = 20) {
  ...
  return query(
    `SELECT c.name, ${DISPLAY.replace('x.customer', 'c.name')} AS display_name, c.phone, c.customer_group,
      ...
      WHERE c.name ILIKE $1 OR c.customer_name ILIKE $1 OR c.contact_name ILIKE $1 OR o.customer_name ILIKE $1
...
export async function detail(id) {
  const header = await one(
    `SELECT c.name, c.customer_name, c.contact_name, c.phone, c.phone_identity, c.customer_group, c.territory, c.first_seen::text AS first_seen,
