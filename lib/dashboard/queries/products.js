...
export async function itemDetail(code, start, end, now = new Date()) {
  ...
  const [trend, channels, customers, invoices, totals] = await Promise.all([
    ...
    query(
      `WITH agg AS (
         SELECT i.customer, COUNT(DISTINCT l.invoice)::int AS invoices,
                COALESCE(SUM(l.amount), 0)::float8 AS revenue, COALESCE(SUM(l.qty), 0)::float8 AS qty,
                MAX(NULLIF(i.customer_full_name, '')) AS customer_full_name
           FROM erp_invoice_items l JOIN erp_invoices i ON i.name = l.invoice
          WHERE ${BASE} AND l.item_code = $1 AND i.posting_date BETWEEN $2::date AND $3::date
          GROUP BY i.customer ORDER BY revenue DESC LIMIT 10
       )
       SELECT a.customer, a.invoices, a.revenue, a.qty, ${displayName({ c: 'c', i: 'a', id: 'a.customer' })} AS display_name
         FROM agg a LEFT JOIN erp_customers c ON c.name = a.customer
        ORDER BY a.revenue DESC`,
      [code, start, end],
    ),
    query(
      `SELECT i.name, i.posting_date::text AS posting_date, i.customer, ${displayName({ c: 'c', i: 'i', id: 'i.customer' })} AS display_name,
