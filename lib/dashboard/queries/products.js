...
export async function itemDetail(code, start, end, now = new Date()) {
  ...
  const [trend, channels, customers, invoices, totals] = await Promise.all([
    ...
    query(
      `SELECT i.customer, COALESCE(NULLIF(c.full_name, ''), NULLIF(i.customer_full_name, ''), c.contact_name, c.customer_name, i.customer) AS display_name,
        ...
        GROUP BY i.customer, c.full_name, c.contact_name, c.customer_name ORDER BY revenue DESC LIMIT 10`,
      [code, start, end],
    ),
    query(
      `SELECT i.name, i.posting_date::text AS posting_date, i.customer, COALESCE(NULLIF(c.full_name, ''), NULLIF(i.customer_full_name, ''), c.contact_name, c.customer_name, i.customer) AS display_name,
