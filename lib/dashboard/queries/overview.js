...
/**
 * Orders for the four fixed periods, all channels, from invoices — one
 * submitted, non-return invoice is one order. With the customers who bought
 * and how many of them were first-time buyers. The app/web/offline split is
 * left to the charts (owner, 2026-09-05).
 */
export async function ordersKpis(now = new Date()) {
  ...
  const count = (start, end) => one(
    `WITH firsts AS (
       SELECT customer, MIN(posting_date) AS first_date FROM erp_invoices WHERE ${SALES_WHERE} GROUP BY customer
     )
     SELECT COUNT(*) FILTER (WHERE NOT COALESCE(i.is_return, false))::int AS orders,
            COUNT(DISTINCT i.customer)::int AS customers,
            COUNT(DISTINCT i.customer) FILTER (WHERE f.first_date BETWEEN $1::date AND $2::date)::int AS new_customers,
            COALESCE(SUM(i.grand_total), 0)::float8 AS value
       FROM erp_invoices i LEFT JOIN firsts f ON f.customer = i.customer
      WHERE i.docstatus = 1 AND i.deleted_at IS NULL AND i.posting_date BETWEEN $1::date AND $2::date`,
    [start, end],
  );
