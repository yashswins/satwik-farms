...
export async function adoptionMonthly(months = 12, now = new Date()) {
  ...
  return query(
    `SELECT date_trunc('month', i.posting_date)::date::text AS month,
            ...
            COUNT(DISTINCT i.customer) FILTER (WHERE i.channel IN ('app', 'web', 'online_unsplit'))::int AS online
       FROM erp_invoices i
...
export async function basketByChannel(start, end, limit = 8) {
  return query(
    `WITH lines AS (
       SELECT CASE WHEN i.channel IN ('app', 'web', 'online_unsplit') THEN 'online' ELSE 'offline' END AS grp,
