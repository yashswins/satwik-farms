...
import { addDays, granularityFor, resolvePeriod, toDate, toIso } from '@/lib/dashboard/periods';
...
/**
 * How the online channel is doing over the four fixed periods (Sales page):
 * orders placed per our own records (app and web) and what invoices — the
 * truth — say was sold online, each with the previous period for comparison.
 */
export async function onlineByPeriod(now = new Date()) {
  const periods = ['today', 'week', 'mtd', 'last30'].map((k) => resolvePeriod(k, { now }));
  const measure = (start, end) => one(
    `WITH inv AS (
       SELECT COUNT(*) FILTER (WHERE i.channel IN ('app', 'web') AND NOT COALESCE(i.is_return, false))::int AS online_invoices,
              COUNT(*) FILTER (WHERE i.channel = 'app' AND NOT COALESCE(i.is_return, false))::int AS app_invoices,
              COUNT(*) FILTER (WHERE i.channel = 'web' AND NOT COALESCE(i.is_return, false))::int AS web_invoices,
              COALESCE(SUM(i.grand_total) FILTER (WHERE i.channel IN ('app', 'web')), 0)::float8 AS online_sales,
              COALESCE(SUM(i.grand_total), 0)::float8 AS sales,
              COUNT(DISTINCT i.customer) FILTER (WHERE i.channel IN ('app', 'web'))::int AS online_customers
         FROM erp_invoices i
        WHERE ${BASE} AND i.posting_date BETWEEN $1::date AND $2::date
     ), placed AS (
       SELECT COUNT(*)::int AS placed,
              COUNT(*) FILTER (WHERE o.channel = 'web')::int AS placed_web,
              COALESCE(SUM(o.total), 0)::float8 AS placed_value
         FROM orders o
        WHERE o.status IN ('pending', 'completed')
          AND (o.created_at AT TIME ZONE 'UTC' AT TIME ZONE 'Africa/Dar_es_Salaam')::date BETWEEN $1::date AND $2::date
     )
     SELECT inv.*, placed.* FROM inv, placed`,
    [start, end],
  );
  const results = await Promise.all(periods.flatMap((p) => [measure(p.start, p.end), measure(p.compareStart, p.compareEnd)]));
