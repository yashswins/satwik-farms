...
export async function attention(days = 14) {
  const since = `(now() AT TIME ZONE 'UTC') - INTERVAL '${Number(days) || 14} days'`;
  const [rejected, failed, queued, noSalesOrder, reconcile, soDeleted, notInvoiced, mismatch, twins] = await Promise.all([
    query(`SELECT ${ORDER_COLS} FROM orders o WHERE o.status = 'rejected' AND o.created_at >= ${since} ORDER BY o.created_at DESC LIMIT 100`),
    query(`SELECT ${ORDER_COLS} FROM orders o WHERE o.status = 'failed' AND o.created_at >= ${since} ORDER BY o.created_at DESC LIMIT 100`),
    query(`SELECT ${ORDER_COLS} FROM orders o WHERE o.status = 'queued' AND o.created_at < (now() AT TIME ZONE 'UTC') - INTERVAL '15 minutes' ORDER BY o.created_at DESC LIMIT 100`),
    query(`SELECT ${ORDER_COLS} FROM orders o WHERE o.status = 'pending' AND o.accu360_order_id IS NULL AND o.created_at < (now() AT TIME ZONE 'UTC') - INTERVAL '60 minutes' AND o.created_at >= ${since} ORDER BY o.created_at DESC LIMIT 100`),
    query(`SELECT ${ORDER_COLS} FROM orders o WHERE o.failure_reason LIKE 'RECONCILE%' AND o.created_at >= ${since} ORDER BY o.created_at DESC LIMIT 100`),
    query(`SELECT ${ORDER_COLS}, d.deleted_on, d.deleted_by FROM orders o JOIN erp_deleted_docs d ON d.deleted_doctype = 'Sales Order' AND d.deleted_name = o.accu360_order_id WHERE o.created_at >= ${since} AND NOT EXISTS (SELECT 1 FROM erp_invoices i WHERE i.sf_order_id = o.id AND i.docstatus = 1) ORDER BY d.deleted_on DESC LIMIT 100`),
    query(`SELECT ${ORDER_COLS}, s.name AS so_name, s.delivery_date::text AS delivery_date, s.status AS so_status FROM orders o JOIN erp_sales_orders s ON s.po_no = o.id AND s.docstatus = 1 AND s.deleted_at IS NULL WHERE o.status = 'pending' AND COALESCE(s.per_billed, 0) = 0 AND s.status NOT IN ('Cancelled', 'Closed', 'Completed') AND s.delivery_date < CURRENT_DATE - 1 AND o.created_at >= ${since} ORDER BY s.delivery_date ASC LIMIT 100`),
    query(`SELECT ${ORDER_COLS}, i.name AS invoice, i.grand_total AS invoiced, i.posting_date::text AS posting_date FROM orders o JOIN erp_invoices i ON i.sf_order_id = o.id AND i.docstatus = 1 AND i.deleted_at IS NULL WHERE o.created_at >= ${since} AND ABS(i.grand_total - o.total) > GREATEST(2000, 0.05 * o.total) ORDER BY o.created_at DESC LIMIT 100`),
    query(`SELECT ${ORDER_COLS} FROM orders o WHERE o.failure_reason LIKE 'DUPLICATE%' AND o.created_at >= ${since} ORDER BY o.created_at DESC LIMIT 50`),
  ]);
...
export async function orderDetail(id) {
  ...
  const [salesOrders, invoices, deleted, acks] = await Promise.all([
    ...
    query("SELECT at, actor, action, detail FROM dashboard_audit WHERE action = 'acknowledge' AND detail->>'order_id' = $1 ORDER BY at DESC", [id]),
  ]);
