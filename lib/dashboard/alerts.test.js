...
describe('evaluateAlerts', () => {
  ...
  it('reads Accu360, restarts, catalogue and backlog from the health snapshot', () => {
    const a = evaluateAlerts({
      sync: syncOk, now: NOW,
      healthSnapshot: fresh({ accu360: { ok: false }, catalog: { configured: true, age_seconds: 3600 }, erp_sync: { pending: 3, escalated: 1, oldest_minutes: 45, alarm: true }, render: { events_24h: { server_failed: 5 } } }),
    });
