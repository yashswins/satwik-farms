...
describe('parseMetricEvent', () => {
  it('accepts every funnel event', () => {
    for (const event of METRIC_EVENTS) {
      expect(parseMetricEvent({ event })).toEqual({ event, returning: false, source: 'web' });
    }
  });
  ...
  it('the phone app declares itself; anything else is the web', () => {
    expect(parseMetricEvent({ event: 'shop_viewed', source: 'app' }).source).toBe('app');
    expect(parseMetricEvent({ event: 'shop_viewed' }).source).toBe('web');
    expect(parseMetricEvent({ event: 'shop_viewed', source: 'bot' }).source).toBe('web');
  });
});
...
describe('metricKeys', () => {
  ...
  it('app events count under their own prefix, never the web counters', () => {
    expect(metricKeys({ event: 'added_to_cart', returning: false, source: 'app' }, '2026-08-03'))
      .toEqual(['sf:m:app:added_to_cart:2026-08-03']);
    expect(metricKeys({ event: 'order_placed', returning: true, source: 'app' }, '2026-08-03'))
      .toEqual(['sf:m:app:order_placed:2026-08-03', 'sf:m:app:order_placed:r:2026-08-03']);
  });
});
