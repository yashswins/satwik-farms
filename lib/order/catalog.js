...
/**
 * Where a banner sends the customer.
 *
 * The sheet describes the destination as `target_type` + `target_id` rather
 * than a URL — verified against the real schema 2026-08-03. A banner with no
 * usable target is not a bug; it simply renders as a non-interactive image
 * rather than a link that goes nowhere.
 */
export function bannerHref(targetType, targetId) {
  const id = String(targetId ?? '').trim();
  if (!id) return null;
  switch (String(targetType ?? '').trim().toLowerCase()) {
    case 'category': return `/order/category/${encodeURIComponent(id)}`;
    case 'product': return `/order/product/${encodeURIComponent(id)}`;
    case 'combo': return `/order/combo/${encodeURIComponent(id)}`;
    case 'section': return '/order';
    // An explicit external link, if the sheet ever carries one.
    case 'url': return id.startsWith('http') ? id : null;
    default: return null;
  }
}

function mapBanner(raw) {
  const targetType = raw.target_type ?? null;
  const targetId = raw.target_id ?? null;
  return {
    id: String(raw.id ?? '').trim(),
    // The live schema carries no title/subtitle; both stay optional so a banner
    // is still readable if the sheet gains them later.
    title: raw.title ?? '',
    subtitle: raw.subtitle ?? null,
    imageUrl: raw.image_url ?? null,
    targetType,
    targetId,
    href: bannerHref(targetType, targetId),
    isActive: truthy(raw.is_active),
    sortOrder: Number(raw.sort_order ?? 0),
  };
}
...
/**
 * Spend-based discount tier, from the sheet's discount_tiers tab.
 *
 * The tab is new (2026-08-06). A feed without it simply yields no tiers, so the
 * code can ship before the sheet changes and nothing breaks either way.
 */
function mapDiscountTier(raw) {
  return {
    id: String(raw.id ?? '').trim(),
    minSpend: Number(raw.min_spend ?? 0),
    percentOff: Number(raw.percent_off ?? 0),
...
/**
 * Promo code as the CLIENT needs it.
 *
 * Only auto-applying codes are safe to send to the browser: they are advertised
 * to everyone anyway. A code the customer must type stays server-side, or the
 * whole list of codes and their minimum spends ships with every page.
 */
function mapPromoCode(raw) {
  return {
    code: String(raw.code ?? '').trim().toUpperCase(),
    amountOff: Number(raw.amount_off ?? 0),
    minimumSpend: Number(raw.minimum_spend ?? 0),
    active: truthy(raw.active),
    autoApply: truthy(raw.auto_apply),
    label: raw.label ?? null,
  };
}
...
/**
 * The single promo the home banner advertises — highest amount_off among
 * active codes, exactly as the phone app picks it (useHome.ts:173-179).
 *
 * Runs SERVER-side (catalogue proxy) against the raw promo list, which never
 * reaches the browser. Advertising the one best code is deliberate publicity —
 * the phone app shows it to everyone — and is not the same thing as shipping
 * the full list with every code and its minimum spend.
 */
export function promoBannerFrom(promoCodes) {
  const active = (promoCodes ?? []).filter((p) => truthy(p?.active));
  if (active.length === 0) return null;
  const best = active.reduce((a, b) =>
    ((Number(b?.amount_off) || 0) > (Number(a?.amount_off) || 0) ? b : a));
  const code = String(best?.code ?? '').trim().toUpperCase();
  const amountOff = Number(best?.amount_off) || 0;
  return code && amountOff > 0 ? { code, amount_off: amountOff } : null;
}

function mapPromoBanner(raw) {
  if (!raw || typeof raw !== 'object') return null;
  const code = String(raw.code ?? '').trim();
  const amountOff = Number(raw.amount_off ?? 0);
  return code && amountOff > 0 ? { code, amountOff } : null;
}
...
export function mapCatalog(raw) {
  ...
  return {
    ...
    discountTiers: (safe.discount_tiers ?? [])
      .map(mapDiscountTier).filter((t) => t.isActive && t.percentOff > 0).sort(bySortOrder),
    // Auto-applying codes only — see mapPromoCode.
    promoCodes: (safe.promo_codes ?? [])
      .map(mapPromoCode).filter((p) => p.active && p.autoApply && p.amountOff > 0),
    promoBanner: mapPromoBanner(safe.promo_banner),
    productsById,
    productsBySku,
